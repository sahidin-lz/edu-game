#!/bin/bash
cat << 'INNER_EOF' > src/components/ActionInput.tsx
import React, { useState, useRef, useEffect } from 'react';
import { Mic, Send, Type, Loader2, Globe, MapPin, AlertTriangle } from 'lucide-react';
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center space-x-4 mb-3 overflow-x-auto pb-2 scrollbar-none">
          ...
          <button
            ...
            className={`flex items-center space-x-2 px-3 py-1.5 rounded text-xs font-bold tracking-widest uppercase transition-colors border whitespace-nowrap ${!isSpeechSupported ? 'opacity-50 cursor-not-allowed' : ''} ${inputType === 'suara_orasi' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-800' : 'text-slate-500 border-transparent hover:text-slate-300'}`}
          >
            <Mic size={14} />
            <span>Orasi</span>
          </button>
        ...
        {!isSpeechSupported && (
          <div className="mb-3 px-3 py-2 bg-red-900/20 border border-red-800 rounded flex items-center gap-2">
            <AlertTriangle size={14} className="text-red-400" />
            <p className="text-[10px] text-red-300 font-mono uppercase">Fitur orasi suara tidak didukung oleh browser Anda. Dialihkan ke mode teks.</p>
          </div>
        )}
        ...
        {groundingResult && (
          <div className="mb-3 p-3 bg-slate-950 border border-slate-800 rounded-md">
            <h4 className="text-[10px] uppercase font-bold tracking-widest text-slate-500 mb-1 flex items-center gap-2">
              <Globe size={12} /> Hasil Riset & Grounding (Gemini 1.5 Flash)
            </h4>
